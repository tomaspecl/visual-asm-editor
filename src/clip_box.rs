...
pub struct Viewport {
    ...
    pub view_size: Size,
    /// When false clamp_view_origin() will not clamp origin such that view rectangle is contained in the content rectangle
    pub do_clamping: bool,
}
...
impl Viewport {
    ...
    pub fn clamp_view_origin(&self, origin: Point) -> Point {
        if !self.do_clamping {return origin}
        let x = origin
            .x
...
impl<T, W> ClipBox<T, W> {
    ...
    /// Set whether the view rectangle will be clamped to be contained in the content rectangle
    pub fn set_do_clamping(&mut self, do_clamping: bool) {
        self.port.do_clamping=do_clamping;
    }
}
...
impl<T: Data, W: Widget<T>> Widget<T> for ClipBox<T, W> {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut T, env: &Env) {
        ...
        let force_event = self.child.is_hot() || self.child.has_active() || !self.port.do_clamping;
